		...
		if (isset($THEMEREX_GLOBALS['post_meta_box']) && $THEMEREX_GLOBALS['post_meta_box']['page']=='post') {
			$THEMEREX_GLOBALS['post_meta_box']['fields'] = array(
					...
					"info_reviews_1" => array(
						"title" => __('Reviews criterias for this post', 'themerex'),
						"override" => "post",
						"divider" => false,
...
if (!function_exists('themerex_post_add_options_column')) {
	//add_filter('manage_edit-{post_type}_columns',	'themerex_post_add_options_column', 9);
	function themerex_post_add_options_column( $columns ){
		$columns['theme_options'] = __('Theme Options', 'themerex');
		return $columns;
	}
}
...
if (!function_exists('themerex_post_fill_options_column')) {
	//add_filter('manage_{post_type}_custom_column',	'themerex_post_fill_options_column', 9, 2);
		...
		if ($props = get_post_meta($post_id, 'post_custom_options', true)) {
			...
			foreach($props as $prop_name=>$prop_value) {
				if (!themerex_is_inherit_option($prop_value) && (!isset($THEMEREX_GLOBALS['options'][$prop_name]['type']) || $THEMEREX_GLOBALS['options'][$prop_name]['type']!='hidden')) {
					...
					$options .= '<div class="themerex_options_prop_row"><span class="themerex_options_prop_name">' . esc_html($prop_title) . '</span>&nbsp;=&nbsp;<span class="themerex_options_prop_value">' . (is_array($prop_value) ? __('[Complex Data]', 'themerex') : '"' . esc_html(themerex_strshort($prop_value, 80)) . '"') . '</span></div>';
				}
			}
		}
...
if (!function_exists('themerex_post_before_show_meta_box')) {
	//add_action('themerex_action_post_before_show_meta_box', 'themerex_post_before_show_meta_box', 10, 2);
	function themerex_post_before_show_meta_box($post_type, $post_id) {
...
if (!function_exists('themerex_post_after_show_meta_box')) {
	//add_action('themerex_action_post_after_show_meta_box', 'themerex_post_after_show_meta_box', 10, 2);
	function themerex_post_after_show_meta_box($post_type, $post_id) {
		?>
		</div>
		<?php
	}
...
if ( !function_exists( 'themerex_post_list_post_types' ) ) {
	...
	function themerex_post_list_post_types($list) {
		$list['post'] = __('Post', 'themerex');
		$list['page'] = __('Page', 'themerex');
		return $list;
	}
}
...
if ( !function_exists( 'themerex_post_get_blog_title' ) ) {
	...
	function themerex_post_get_blog_title($title, $page) {
		...
		if ( $page == 'blog' )
			$title = __( 'All Posts', 'themerex' );
		else if ( $page == 'author' ) {
			$curauth = (get_query_var('author_name')) ? get_user_by('slug', get_query_var('author_name')) : get_userdata(get_query_var('author'));
			$title = sprintf(__('Author page: %s', 'themerex'), $curauth->display_name);
		} else if ( $page == 'error404' )
			$title = __('URL not found', 'themerex');
		else if ( $page == 'search' )
			$title = sprintf( __( 'Search Results for: %s', 'themerex' ), get_search_query() );
		else if ( $page == 'archives_day' )
			$title = sprintf( __( 'Daily Archives: %s', 'themerex' ), themerex_get_date_translations(get_the_date()) );
		else if ( $page == 'archives_month' )
			$title = sprintf( __( 'Monthly Archives: %s', 'themerex' ), themerex_get_date_translations(get_the_date( 'F Y' )) );
		else if ( $page == 'archives_year' )
			$title = sprintf( __( 'Yearly Archives: %s', 'themerex' ), get_the_date( 'Y' ) );
		 else if ( $page == 'category' )
			$title = sprintf( __( '%s', 'themerex' ), single_cat_title( '', false ) );
		else if ( $page == 'tag' )
			$title = sprintf( __( 'Tag: %s', 'themerex' ), single_tag_title( '', false ) );
		else if ( $page == 'attachment' )
			$title = sprintf( __( 'Attachment: %s', 'themerex' ), themerex_get_post_title());
		else if ( $page == 'single' )
