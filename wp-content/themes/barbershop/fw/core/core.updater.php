...
if (!function_exists('themerex_updater_theme_setup')) {
	...
	function themerex_updater_theme_setup() {
		...
		//add_action('admin_menu',		'themerex_update_notifier_menu');  

		// Add an update notification to the WordPress 3.1+ Admin Bar
		add_action( 'admin_bar_menu',	'themerex_update_notifier_bar_menu', 1000 );
	}
}
if (!function_exists('themerex_updater_after_theme_setup')) {
	...
	function themerex_updater_after_theme_setup() {
		...
		$THEMEREX_GLOBALS['update_notifier_options'] = apply_filters('themerex_filter_update_notifier', array(
			...
			'xml_url'		=> 'http://themerex.net/!updates/',		// Folder URL to the remote notifier XML file containing the latest version of the theme and changelog (file name must be equal with theme folder name)
	...
	function themerex_update_notifier_menu() {  
		if (function_exists('simplexml_load_string')) {					// Stop if simplexml_load_string function isn't available
			...
			if( (string) $xml->latest > (string) $theme_data->get('Version') ) { // Compare current theme version with the remote XML version
				//add_dashboard_page( $THEMEREX_GLOBALS['update_notifier_options']['theme_name'] . ' ' . __('Theme Updates', 'themerex'), $THEMEREX_GLOBALS['update_notifier_options']['theme_name'] . ' <span class="update-plugins count-1"><span class="update-count">'.__('New Update', 'themerex').'</span></span>', 'administrator', 'theme-update-notifier', 'update_notifier');
			}
		}	
	}
}
...
if (!function_exists('themerex_update_notifier_bar_menu')) {
	...
	function themerex_update_notifier_bar_menu() {
		if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
			...
			$theme_data = wp_get_theme();
			if( (string) $xml->latest > (string) $theme_data->get('Version') ) { // Compare current theme version with the remote XML version
				$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>'.($THEMEREX_GLOBALS['update_notifier_options']['theme_name']).' <span id="ab-updates">'.sprintf(__('New Update: %s', 'themerex'), $xml->latest).'</span></span>', 'href' => get_admin_url() . 'index.php?page=theme-update-notifier' ) );
			}
		}
	}
}
...
if (!function_exists('themerex_update_notifier')) {
	...
	?>
		...
		<div class="wrap">
			...
			<h2><?php echo ($THEMEREX_GLOBALS['update_notifier_options']['theme_name']); ?> <?php _e('Theme Updates', 'themerex'); ?></h2>
			<div id="message" class="updated below-h2"><p><strong><?php echo sprintf(__('There is a new version of the %s theme available.', 'themerex'), $THEMEREX_GLOBALS['update_notifier_options']['theme_name']); ?></strong> <?php echo sprintf(__('You have version %s installed. Update to version %s.', 'themerex'), $theme_data->get('Version'), $xml->latest); ?></p></div>
	
			<div><img alt="" style="margin: 0 0 20px 0; border: 1px solid #ddd;" src="<?php echo esc_url(themerex_get_file_url('/screenshot.png')); ?>" /></div>
			
			<div id="theme_update_instructions">
				<h3><?php _e('Update Download and Instructions', 'themerex'); ?></h3>
				<p><?php echo sprintf(__('<strong>Please note:</strong> make a backup of the Theme inside your WordPress installation folder <strong>/wp-content/themes/%s/</strong>', 'themerex'), $THEMEREX_GLOBALS['update_notifier_options']['theme_folder']); ?></p>
