	...
	function import( $file ) {
		...
		try {
			if ($this->start_from_id == 0) {
				echo '<script>'
					. 'document.getElementById("import_progress_status").innerHTML = "' . __('Import categories ...', 'themerex') .'";'
					. '</script>';
				echo '<br><b>'.__('Import categories ...', 'themerex').'</b><br>'; flush();
				$this->process_categories();
				echo '<script>'
					. 'document.getElementById("import_progress_status").innerHTML = "' . __('Import tags ...', 'themerex') .'";'
					. '</script>';
				echo '<br><b>'.__('Import tags ...', 'themerex').'</b><br>'; flush();
				$this->process_tags();
				echo '<script>'
					. 'document.getElementById("import_progress_status").innerHTML = "' . __('Import terms ...', 'themerex') .'";'
					. '</script>';
				echo '<br><b>'.__('Import terms ...', 'themerex').'</b><br>'; flush();
				$this->process_terms();
			}
			$msg = sprintf(__('Import posts %s ...', 'themerex'), ($this->start_from_id > 0 ? sprintf(__('(continue from ID = %s)', 'themerex'), $this->start_from_id) : ''));
			echo '<script>'
		...
		if ($result >= 100) {
			echo '<script>'
				. 'document.getElementById("import_progress_status").innerHTML = "' . __('Backfill parents and attachments ...', 'themerex') .'";'
				. '</script>';
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			echo '<script>'
				...
				. 'document.getElementById("import_progress_status").innerHTML = "' . sprintf( __('Post (ID=%s) imported. Elapsed time: %s sec.', 'themerex'), $post['post_id'],  time() - $this->start_time) .'";'
				. '</script>';
			echo "<br />";
