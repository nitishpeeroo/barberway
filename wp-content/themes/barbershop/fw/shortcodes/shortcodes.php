...
function themerex_sc_accordion($atts, $content=null){	
    ...
    extract(themerex_sc_html_decode(shortcode_atts(array(
		...
		"initial" => "1",
		"counter" => "off",
		"icon_closed" => "icon-minus-2",
		"icon_opened" => "icon-plus-2",
		// Common params
		"id" => "",
		"class" => "",
		"css" => "",
	...
	$THEMEREX_GLOBALS['sc_accordion_icon_closed'] = empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed) ? "icon-minus-2" : $icon_closed;
	$THEMEREX_GLOBALS['sc_accordion_icon_opened'] = empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened) ? "icon-plus-2" : $icon_opened;
...
function themerex_sc_accordion_item($atts, $content=null) {
	...
	if (empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed)) $icon_closed = $THEMEREX_GLOBALS['sc_accordion_icon_closed'] ? $THEMEREX_GLOBALS['sc_accordion_icon_closed'] : "icon-minus-2";
	if (empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened)) $icon_opened = $THEMEREX_GLOBALS['sc_accordion_icon_opened'] ? $THEMEREX_GLOBALS['sc_accordion_icon_opened'] : "icon-plus-2";
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
...
/*
[trx_audio url="http://yoursite/wp-content/uploads/2014/12/Dream-Music-Relax.mp3" image="http://yoursite/wp-content/uploads/2014/10/post_audio.jpg" title="Insert Audio Title Here" author="Lily Hunter" controls="show" autoplay="off"]
*/
		...
		. ($content != '' && themerex_sc_param_is_on($custom)
			...
			: '<div class="sc_contact_form_info">'
              ...
              .'</div>'
                    ...
                    .'<div class="sc_contact_form_item sc_contact_form_button"><button>'.__('SEND', 'themerex').'</button></div>'
			)
...
function themerex_sc_image($atts, $content=null){	
	...
	$output = empty($src) ? '' : ('<figure' . ($id ? ' id="'.esc_attr($id).'"' : '') 
		...
		. '>'
            ...
            . ((themerex_sc_param_is_on($link) && $linkurl) ? '<a href="'. esc_url($linkurl) .'"><span>view</span></a>' : '' )
				...
				. ' title="' . __('Open/close search form', 'themerex') . '">
					...
					<div class="search_form_wrap">
						<form role="search" method="get" class="search_form" action="' . esc_url( home_url( '/' ) ) . '">
							<button type="submit" class="search_submit icon-magnifying-glass-2" title="' . __('Start search', 'themerex') . '"></button>
	...
	if (themerex_sc_param_is_on($custom) && $content) {
		...
		while ( $query->have_posts() ) { 
			...
			if (empty($photo)) {
				...
				$photo = themerex_get_resized_image_tag($photo, 350, 410);
			}
			$socials = '';
							...
							. '<div' . ($id ? ' id="'.esc_attr(($id).'_'.($post_number)).'"' : '') 
								...
								. '<div class="sc_team_item_avatar">'
									...
									. ($style==2 
										...
										: '')
                                    .'<a class="appoinment_button" href="http://barber.axiomthemes.com/book/">appointment</a>'
								. '</div>'
	...
	if (themerex_sc_param_is_on($custom) && $content) {
		...
		while ( $query->have_posts() ) { 
            ...
            $output .= '<div class="swiper-slide" data-style="'.esc_attr(($ws).($hs)).'" style="'.esc_attr(($ws).($hs)).'">'
                ...
                . ($author ? '<div class="sc_testimonial_author">' . ($link ? '<a href="'.esc_url($link).'">'.esc_html($author).'</a>' : $author) . '</div>' : '')
                . '</div>'
                . '</div>'
                . '</div>';
		}
...
function themerex_sc_toggles($atts, $content=null){	
	...
	$THEMEREX_GLOBALS['sc_toggles_icon_closed'] = empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed) ? "icon-minus-2" : $icon_closed;
	$THEMEREX_GLOBALS['sc_toggles_icon_opened'] = empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened) ? "icon-plus-2" : $icon_opened;
...
function themerex_sc_toggles_item($atts, $content=null) {
	...
	if (empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed)) $icon_closed = $THEMEREX_GLOBALS['sc_toggles_icon_closed'] ? $THEMEREX_GLOBALS['sc_toggles_icon_closed'] : "icon-minus-2";
	if (empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened)) $icon_opened = $THEMEREX_GLOBALS['sc_toggles_icon_opened'] ? $THEMEREX_GLOBALS['sc_toggles_icon_opened'] : "icon-plus-2";
