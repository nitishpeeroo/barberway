		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email])) {
				if ($subscribers[$group][$email] == $nonce) {
					...
					update_option('themerex_emailer_subscribers', $subscribers);
					themerex_set_system_message(__('Confirmation complete! E-mail address succefully added in the subscribers list!', 'themerex'), 'success');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				} else if ($subscribers[$group][$email] != '') {
					themerex_set_system_message(__('Bad confirmation code!', 'themerex'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				} else {
					themerex_set_system_message(__('E-mail address already exists in the subscribers list!', 'themerex'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				}
			}
